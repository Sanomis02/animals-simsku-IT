...
class AnketaController
{
    ...
    public function loadToFormForAdd($request, $response, $args)
    {
        ...
        if (($_SESSION["prieiga"]=="Administratorius")||($_SESSION["prieiga"]=="Kontrolierius")
		     || ($_SESSION["prieiga"] == "Prisiregistraves" && $_SESSION["canPost"] == 1) ) {
            $this->defaultData();
			...
			$this->addFlashMessage("anketa_failure", "Neturite teisių kurti skelbimo/anketos, susisiekite su administratoriumi");
    ...
    public function addFromForm($request, $response, $args)
    {
        if ($request->isPost()) {
            ...
            $autorius_id = intval($_SESSION["vartotojas_id"]);
//                $pagr_nuotraukos_id = $postValues["pagr_nuotraukos_id"];
            ...
            $fk_Naudotojasid = intval($_SESSION["vartotojas_id"]);
...
//                    $pagr_nuotraukos_id,
                ...
                $aprasymas,
                $dingimo_data);
            $this->addFlashMessage("success", "Gyvuno anketa sukurta, jo id {$naujos_anketos_id} jo vardas {$gyvuno_vardas} dabar galite prisegti nuotraukas");
        }
    ...
    {
		$anketa = $this->getOneANketa($args['id']);
		
        if (($_SESSION["prieiga"]=="Administratorius")||($_SESSION["prieiga"]=="Kontrolierius")
		     || ($_SESSION["prieiga"] == "Prisiregistraves" && $_SESSION["vartotojas_id"] == $anketa->fk_Naudotojasid) ) {
            $this->defaultData();
            $this->addData("prieiga", $_SESSION["prieiga"]);
...
//                var_dump($pagr_nuotraukos_id);die;
                ...
                $this->atnaujintiAnketa($args["id"],
                    ...
                    $aprasymas,
                    $dingimo_data);
                $this->addFlashMessage("success", "Gyvunas atnaujintas jo id {$args['id']} jo vardas {$gyvuno_vardas} dabar galite sukurti skelbimą");
