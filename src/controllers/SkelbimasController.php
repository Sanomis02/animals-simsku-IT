...
//        var_dump($_SESSION["paiesk_fragm_sesija"]);die;
	...
	public function removeSkelbima($request, $response, $args)
	{
    ...
    public function loadToFormForAdd($request, $response, $args)
    {
        ...
        if (($_SESSION["prieiga"]=="Administratorius")||($_SESSION["prieiga"]=="Kontrolierius")
            ...
            $this->defaultData();
            $this->addData("prieiga", $_SESSION["prieiga"]);
			$this->addData("vartotojo_id", $_SESSION["vartotojas_id"]);
    ...
    public function loadToFormForEdit($request, $response, $args)
    {
        if (($_SESSION["prieiga"]=="Administratorius")||($_SESSION["prieiga"]=="Kontrolierius")
			...
			$this->addFlashMessage("skelbimo_edit_failure", "Nepavyko redaguoti skelbimo");
...
//                var_dump($postValues);die;
                ...
                $naujo_skelbimo_id = $this->addNewRecordInSkelbimuTable(
                    ...
                    $perziuros_kiekis,
                    $aprasymas);
                $this->addFlashMessage("success", "Skelbimas pridėtas jo id {$naujo_skelbimo_id} galite grįžti į pagrindinį puslapį");
...
//                var_dump($postValues);die;
    ...
    public function delete($request, $response, $args)
    {
       if($request->isPost() && $request->isXhr()) {
            $body = $request->getParsedBody();
            if(isset($body["skelbimoId"])) {
               $this->deleteAnketa($body["skelbimoId"]);
			   $this->addFlashMessage("delete", "Sėkmingai ištrintas skelbimas");
            }
        }
	}
    ...
    private function deleteAnketa($id)
    {
