...
class VartotojasController
{
    ...
    public function login($request, $response, $args)
    {
		...
		$this->addData("errorMessage",$this->getFlashMessage("failure")[0]);
        $data = $this->getData();
    ...
    public function authenticateUser($request, $response, $args)
    {
        ...
        if($request->isPost()){
           ...
           if($vartotojas = $this->checkVartotoja($pastas, $slaptazodis)) {
			   $_SESSION["vartotojas_id"] = $vartotojas->getId();
		       ...
		       $_SESSION["canPost"] = $vartotojas->getAr_gali_skelbt();
               }
               return $response->withRedirect('/',301);
           } else {
			   $this->addFlashMessage("failure", "Netinkamas slaptažodis arba paskyros vardas");
...
//            var_dump($_SESSION["prieiga"]);
    ...
    {
        $query = $this->db->prepare('SELECT * FROM Vartotojai WHERE `pastas`=:pastas');
        $query->execute([
            "pastas"=>$pastas ]);
        $result = $query->fetch();
        ...
        if($result["id"] >= 1)
        {
            ...
            $tikrinamas->create($result);
			if(password_verify($slaptazodis, $tikrinamas->getSlaptazodis()))
			{
				return $tikrinamas;
			}
        }
    ...
    public function addFromForm($request, $response, $args)
    {
        if($request->isPost()) {
		    ...
		    $slaptazodis = password_hash($postValues["slaptazodis"], PASSWORD_DEFAULT);
    ...
    {
        $this->defaultData();
		if( $_SESSION["prieiga"] != "Administratorius" )
		{
		    $this->addFlashMessage("failure", "Įvyko klaida, susisiekite su administratoriumi");
			$this->addData("errorMessage",$this->getFlashMessage("failure")[0]);
